...
class Graph{
        ...
        void dfs(int a,vector<int> &v, int mode,int &sumred, int &sumblue);
...
void Graph::dfs(int a,vector<int> &v, int mode,int &sumred, int &sumblue){
        visited[a] = true;
        cerr<<a<<" "<<mode<<"\n";
        //cout<<a<<" ";
        if(mode) sumblue += v[a];
        ...
        for(auto t : adj[a]){
                ...
                dfs(t,v,1LL^mode,sumred,sumblue);
        }
        return;
}
...
void solve() {
        ...
        int sumred = 0;
        int sumblue = 0;
        g.dfs(0,v,0,sumred,sumblue);
        cout<<max(sumred,sumblue)<<"\n";
