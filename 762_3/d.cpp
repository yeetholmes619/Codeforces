...
void solve(){
        ...
        for(int j = 0; j < n; j++){
                ...
                for(int i =0 ; i < m; i++){
                        ...
                        else if(v[i][j] == temp.val){
                                (temp.pos).pb(i);
                        }
                }
                debug(j,temp.val,temp.pos);
                pp.pb(temp);
        }
        sort(allvec(pp),cmp);
        set<int> shops;
        ...
        int ans = 1e18;
        for(int i = 0; i < n; i++){
                int kk = (pp[i].pos).size();
                bool ok = false;
                for(int j= 0; j < kk; j++){
                        if(check[(pp[i].pos)[j]]){
                                ok = true;
                                ans = min(ans,pp[i].val);
                                break;
                        }
                }
                if(!ok){
                        if(shops.size() < n-1){
                                shops.insert((pp[i].pos)[0]);
                                check[(pp[i].pos)[0]] = true;
                                ans = min(ans,pp[i].val);
                        }
                        else{
                                debug(i,shops);
                                int tiddies = -1;
                                for(auto it: shops){
                                        tiddies = max(tiddies,v[it][pp[i].index]);
                                }
                                ans = min(ans,tiddies);
                        }
