...
void solve(){
        ...
        vector<int> dead(n,0);
        dead[0] = (v[0] <= temp[0]);
        ans[0] = min(temp[0],v[0]);
        int k = lower_bound(allvec(temp),v[0]) - temp.begin();
        if(k < n and k > 0){
                dead[k]++;
                ans[k] += v[0]-temp[k-1];
        }
        for(int i = 1; i < n; i++){
                int j = lower_bound(i+allvec(temp),temp[i-1]+v[i]) - temp.begin();
                if(j < n){
                        dead[j]++;
                        ans[j] += v[i]+temp[i-1]-temp[j-1];
                }
        }
        debug(dead);
        debug(ans);
        for(int i = 1; i < n; i++) dead[i] += dead[i-1];
        for(int i =1 ; i < n; i++){
                ans[i] += (i+1-dead[i])*(temp[i]-temp[i-1]);
        }
